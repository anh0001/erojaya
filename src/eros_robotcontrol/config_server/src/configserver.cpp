...
bool ConfigServer::handleSave(SaveRequest& req, SaveResponse& resp)
{
	...
	for(ParameterMap::iterator it = m_params.begin(); it != m_params.end(); ++it)
	{
		...
		{
			em << YAML_0_3::Key << tokens[tok_idx];
			em << YAML_0_3::Value << YAML_0_3::BeginMap;

			current_path.push_back(tokens[tok_idx]);
		}

		em << YAML_0_3::Key << prop_name;
		em << YAML_0_3::Value << it->second.value;
	}
	em << YAML_0_3::EndMap;
...
void ConfigServer::insertFromYAML(const YAML_0_3::Node& n, const std::string& path)
{
	if(n.Type() == YAML_0_3::NodeType::Map)
	{
	...
	if(n.Type() == YAML_0_3::NodeType::Scalar)
	{

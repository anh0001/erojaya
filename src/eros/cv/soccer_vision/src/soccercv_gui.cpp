...
//#define SUB_SAMPLING_PARAMETER 2
#define ORG_IMAGE_WIDTH 424
#define ORG_IMAGE_HEIGHT 240
#define SUB_SAMPLING_PARAMETER 1
#define SUB_SAMPLING_WIDTH ORG_IMAGE_WIDTH/SUB_SAMPLING_PARAMETER
#define SUB_SAMPLING_HEIGTH ORG_IMAGE_HEIGHT/SUB_SAMPLING_PARAMETER

#define TOTAL_SUB_IMAGES 8

guchar imgorgbuffer[ORG_IMAGE_WIDTH*ORG_IMAGE_HEIGHT*3];
guchar imgorgsegmented[ORG_IMAGE_WIDTH*ORG_IMAGE_HEIGHT*3];
guchar subimages1[SUB_SAMPLING_WIDTH*ORG_IMAGE_HEIGHT*3];
...
struct orgsizeimage {
    ...
    GdkPixbuf * pixbuf;
	guchar img[ORG_IMAGE_WIDTH*ORG_IMAGE_HEIGHT*3];
};
...
int main( int   argc, char *argv[])
{
    ...
    for(int i = 0; i < ORG_IMAGE_WIDTH*ORG_IMAGE_HEIGHT*3; i++ ){ imgorgbuffer[i] = 128;}

    //Fill org segmented image buffer with white
    for(int i = 0; i < ORG_IMAGE_WIDTH*ORG_IMAGE_HEIGHT*3; i++ ){ imgorgsegmented[i] = 128;}
    ...
    orgpixbuf = gdk_pixbuf_new_from_data (imgorgbuffer, GDK_COLORSPACE_RGB,
		                                  FALSE, 8, ORG_IMAGE_WIDTH, ORG_IMAGE_HEIGHT, ORG_IMAGE_WIDTH*3, NULL, NULL);
    ...
    gtk_widget_set_size_request (boxv1, 650, 720);

    boxv2 = gtk_vbox_new (TRUE,0);
    gtk_widget_set_size_request (boxv2, 450, 720);

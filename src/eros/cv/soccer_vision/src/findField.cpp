...
void FindField::findField(FrameGrabber & CamFrm){
			...
			const unsigned char MIN_COLOR_INTENSITY = 1;
...
// 			cv::waitKey(5);
			...
			int j;
			float gradien=0,lgradien=0;
			CamFrm.FieldPosCount = 0;
			for (j=0; j<CamFrm.soccerFieldHull.size()-1; j++){
				...
				if(j==0){
					CamFrm.soccerFieldPos[0]=pix1;
					CamFrm.FieldPosCount++;
				}else{
					if(abs(lgradien-gradien)>=0.1){
						CamFrm.soccerFieldPos[CamFrm.FieldPosCount]=pix1;
						CamFrm.soccerFieldPos[CamFrm.FieldPosCount+1]=pix2;
						CamFrm.FieldPosCount++;
					}					
				}
				lgradien=gradien;

				//CamFrm.soccerFieldPos[j]=pix1;
				//CamFrm.soccerFieldPos[j+1]=pix2;
				//CamFrm.FieldPosCount++;
				////////////////////////////////////////////////////////////////////////

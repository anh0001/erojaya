...
{
	std::string name = node["name"].to<std::string>();
	const YAML::Node& mean = node["mean"];
	const YAML::Node& cov = node["cov"];
	const YAML::Node& Yrange = node["yrange"];
	float angle = node["angle"].to<float>();
	int id = node["colorID"].to<int>();

	e.setMean(QPointF(mean[0].to<float>(), mean[1].to<float>()));
	e.setCov(cov[0].to<float>(), cov[1].to<float>());
	e.setAngle(angle);
	e.setColorID(id);
	e.setName(QString(name.c_str()));
	e.setYRange((int)Yrange[0].to<int>(), (int)Yrange[1].to<int>());

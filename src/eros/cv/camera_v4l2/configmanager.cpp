...
static int enumerateMenu(int fd, calibration::CameraParam* param, const v4l2_queryctrl& query)
{
	v4l2_querymenu qm;
	...
	for(qm.index = query.minimum; qm.index <= (uint)query.maximum; qm.index++)
	{
		...
		calibration::CameraParamChoice choice;
...
calibration::CameraParam* ConfigManager::getParamInfo(const std::string& name)
{
...
calibration::CameraParam* ConfigManager::getParamInfo(int id)
{
	for(uint i = 0; i < m_controls.params.size(); ++i)
	{
...
int ConfigManager::readFromCamera(int fd)
{
	...
	for(uint i = 0; i < m_controls.params.size(); ++i)
	{
		calibration::CameraParam* param = &m_controls.params[i];
...
void ConfigManager::writeConfigFile(int fd, std::ostream* ostream)
{
	...
	for(uint i = 0; i < m_controls.params.size(); ++i)
	{
		const calibration::CameraParam& param = m_controls.params[i];

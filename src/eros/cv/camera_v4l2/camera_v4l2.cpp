...
PLUGINLIB_EXPORT_CLASS(CameraV4L2, nodelet::Nodelet)
...
void CameraV4L2::initCamera()
{
	...
	for(int i = 0; i < 10; ++i)
	{
		...
		if(m_fd >= 0)
		{
			NODELET_INFO("Using device: %s", buf);
			snprintf(devx,sizeof(devx),"%s",buf);
			//NODELET_INFO("Use: <<<<<<<<<<<<<<<<<<<<<<< %s",devx);
			break;
		}
	}
	...
	if(m_fd < 0)
	{
		counter_param=0;
		param_hit=0;
		NODELET_ERROR("Could not open device. Retrying in 5 seconds");
	...
	//////////////////////////////////////////////////////////////////////////////////////////
	//Camera Reset to Aperture mode and auto white balance
	counter_param=0;
	param_hit=0;
	sprintf(devz,"v4l2-ctl -d %s --set-ctrl exposure_auto=3",devx);
	system(devz);
	sprintf(devz,"v4l2-ctl -d %s --set-ctrl white_balance_temperature_auto=0",devx);
	system(devz);
	/////////////////////////////////////////////////////////////////////////////////////////
...
void CameraV4L2::update()
{
	...
	if(m_config.writePending() && param_hit==1)
	{
	...
	//NODELET_INFO("Counter: %4d , Hit: %d",counter_param,param_hit);

	//////////////////////////////////////////////////////////////////////////////////////////
	//Camera Set to Manual mode after 50 iteration
	if(param_hit==0)counter_param++;
	if(counter_param>=50){
		sprintf(devz,"v4l2-ctl -d %s --set-ctrl exposure_auto=1",devx);
		system(devz);
		sprintf(devz,"v4l2-ctl -d %s --set-ctrl white_balance_temperature_auto=0",devx);
		system(devz);
...
bool CameraV4L2::srvEnumerateParams(calibration::EnumerateCameraParamsRequest& req, calibration::EnumerateCameraParamsResponse& resp)
{
...
bool CameraV4L2::srvSetParam(calibration::SetCameraParamRequest& req, calibration::SetCameraParamResponse& resp)
{

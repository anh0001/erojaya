...
void ImageWidget::updateWith(const sensor_msgs::Image::Ptr& img)
{
	...
	{
		ROS_INFO_ONCE("Image encoding YUYV");
	...
	{
		ROS_INFO_ONCE("Image encoding UYVY");
		m_img = QImage(img->width, img->height, QImage::Format_RGB32);
	...
	{
		ROS_INFO_ONCE("Image encoding sensor_msgs::image_encodings::MONO8");
		m_img = QImage(img->width, img->height, QImage::Format_RGB32);
		for(size_t y = 0; y < img->height; ++y)
		{
	...
	{	//ROS_INFO("width=%d,height=%d",img->width,img->height);
		//ROS_INFO("step=%d", img->step);
		ROS_INFO_ONCE("Image encoding sensor_msgs::image_encodings::RGB8");
		m_img = QImage(img->width, img->height, QImage::Format_RGB32);
		for(size_t y = 0; y < img->height; ++y)
		{
			for(size_t x = 0; x < img->width; ++x)
			{
				unsigned char* base = &img->data[y*img->step + x*3];
				uint8_t r = base[0];
				uint8_t g = base[1];
				uint8_t b = base[2];
	...
	{
		ROS_INFO_ONCE("Image encoding classes");
		m_img = QImage(img->width, img->height, QImage::Format_RGB32);
		for(size_t y = 0; y < img->height; ++y)
		{
			for(size_t x = 0; x < img->width; ++x)
	...
	{
		ROS_WARN_ONCE("Not listed image encoding");

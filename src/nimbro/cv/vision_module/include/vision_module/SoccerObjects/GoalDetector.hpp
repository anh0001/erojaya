...
public:
			...
			vector<Point2f> &goalPosition, bool SHOWGUI, Mat &guiImg)
	{
		...
		//TODO: Check this... Replaced with binary image from thresholded HSV.
		// experiment-1: It is more robust using canny. Using color hsv, white wall is detected as a goal.
		//HoughLinesP(cannyImg, linesP, 1, M_PI / 45, 10, MinLineLength, 20);
		HoughLinesP(binaryFrame, linesP, 1, M_PI / 45, 10, MinLineLength, 20);
		...
		for (size_t i = 0; i < allVerLines2.size(); i++)
		{
			...
			if (downDistance2Field < MaxOutField)
				continue;
			//TODO: Check this!!! as the robot has to defend half of field
			//if (GetDistance(downReal) > 5)
			if (GetDistance(downReal) > 7)
				continue;

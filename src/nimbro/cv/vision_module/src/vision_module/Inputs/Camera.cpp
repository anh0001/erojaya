...
{
	if(!hitTarget){
		ROS_INFO("SET_AUTO GUVCVIEW");
		if(hit<1){
			for (int i = 0; i < 1; i++){
				sprintf(devz,"v4l2-ctl -d %s --set-ctrl exposure_auto=3",devStr);
				system(devz);
				sprintf(devz,"v4l2-ctl -d %s --set-ctrl white_balance_temperature_auto=0",devStr);
				system(devz);
			}
		}
		hit++;		
	...
	if(hit>25){
		hitTarget=true;
		hit=0;
		ROS_INFO("SET_MANUAL GUVCVIEW");
		for (int i = 0; i < 3; i++)
		{
			sprintf(devz,"v4l2-ctl -d %s --set-ctrl exposure_auto=1",devStr);
			system(devz);
		}
	}
	if (devNumber < 0)
...
bool Camera::changeDevNum(bool first, bool &changed)
{
	...
	string configPath = ros::package::getPath("launch") + "/config/visions/";

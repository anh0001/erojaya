...
bool Camera::SetCameraSetting()
{
	...
	bool res = true;
	// In default camera config, we activate auto exposure
	if (-1 == system(paramDefStr))
	{
		ROS_WARN_THROTTLE(1, "Can't Open Camera Device! Set default camera setting failed.");
		res = false;
	}
	usleep(50000);
	...
	int exposure;
	if (res)
		exposure = (int) cap.get(CV_CAP_PROP_EXPOSURE);

	for (int i = 0; i < 3; i++)
	{
		if (-1 == system(paramStr))
		{
			ROS_WARN_THROTTLE(1, "Can't Open Camera Device! Set camera setting failed");
			res = false;
		}
		Mat tmp;
		cap >> tmp;
	...
	// Apply exposure value from enabled auto-exposure
	if (res)
		if (cap.set(CV_CAP_PROP_EXPOSURE, exposure))
			ROS_WARN("Could not set exposure value %d.", exposure);

		...
		vector<Point> &resCountour)
{
	...
	for (uint32_t i = 0; i < contour.size(); i++)
	{
		...
		if (x < 0 || y < 0 || x >= W || y >= H)
		{
			ROS_ERROR("UndistortP_1: Error In Programming (x =%d, y =%d)", x, y);
			return false;
		}
		...
		vector<Point2f> &resCountour)
{
	...
	for (uint32_t i = 0; i < contour.size(); i++)
	{
		...
		if (x < 0 || y < 0 || x >= W || y >= H)
		{
			ROS_ERROR("UndistortP_2: Error In Programming (x =%d, y =%d)", x, y);
			return false;
		}
...
bool DistortionModel::UndistortP(const Point &inPoint, Point2f &resPoint)
{
	...
	if (x < 0 || y < 0 || x >= W || y >= H)
	{
		ROS_ERROR("UndistortP_3: Error In Programming (x =%d, y =%d)", x, y);
		return false;
	}
...
void DistortionModel::CreateUndistort(const Mat &rawImg, Mat &res)
{
	...
	for (int i = 0; i < rawSize; i++)
	{
		...
		{
			ROS_ERROR("CreateUndistort: Error In Programming %d %d ", x, y);
			continue;
		}
...
bool DistortionModel::Init()
{
	...
	//ROS_INFO("maxW=%d maxH=%d", maxW, maxH);
...
//		//TODO: Check this!!!
//		// We found a negative value of undistorted pos.
//		// The value is mostly -1 on y pos.
//		// This could be attributed to int conversion.
//		if((x + offsetx) < 0 || (y + offsety) < 0)
//		{
//			ROS_WARN("DistortionModel::Init: Found negative pos x=%d y=%d",(x + offsetx),(y + offsety));
//			ROS_WARN("Fix negative value of undistortion");
//			distortionVector[(yP * W) + xP] = Point2f(max(x + offsetx,0), max(y + offsety,0));
//		}
	}

	ROS_INFO("Calculated Boundry if symetrical => ( width = %d , height = %d)",

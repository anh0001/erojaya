						...
						hullField))
				{
					...
					if (params.ball->enable.get())
					{
						...
						if (ballResults.size() > 0)
						{
							...
							outputs.ball.position.z = 0;
							outputs.ball.center_x_im = ballResults[0].Center.x;
							outputs.ball.center_y_im = ballResults[0].Center.y;
							outputs.ball.radius_im = ballResults[0].Radius;
					...
					if (params.loc->enable.get())
					{
								...
								confiused, AllLines, AllFeature))
						{
							...
							for (size_t i = 0; i < AllLines.size(); i++)
							{
								...
								tmpMarker.setColor(1. * (AllLines[i].type / (double) LTRES),
										1. * (1 - (AllLines[i].type	/ (double) LTRES)),

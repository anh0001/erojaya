...
public:
	...
	Settings() :
			...
			aspectRatio(1.0)  // default aspect ratio when CALIB_FIX_ASPECT_RATIO is activated.
			//aspectRatio(4.0/3.0)  //TODO: Check this since our robot uses 4:3 aspect ratio.
	{
	...
	{
		int flag = 0;
//		flag |= CALIB_USE_INTRINSIC_GUESS; // keep camera matrix as zero
...
//		flag |= CALIB_FIX_K3;
		...
		flag |= CALIB_RATIONAL_MODEL;  // Coefficients k4, k5, and k6 are enabled
		flag |= CALIB_THIN_PRISM_MODEL; // Coefficients s1, s2, s3 and s4 are enabled
		flag |= CALIB_FIX_S1_S2_S3_S4;
//		flag |= CALIB_TILTED_MODEL;
//		flag |= CALIB_FIX_TAUX_TAUY;
		return flag;
	}
...
int main(int argc, char* argv[])
{
	...
	while (ros::ok())
	{
		if (cam->TakeCapture() > 0.5)
		{
			...
			Point textOrigin(view.cols - 3 * textSize.width - 10,
					view.rows - 12 * baseLine - 10);
		...
		Mat& distCoeffs, vector<vector<Point2f> > imagePoints)
{
	...
	if (ok)
		...
		cout << "Calibration failed" << ". avg re projection error = " << totalAvgErr << endl;

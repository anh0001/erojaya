		...
		Mat &guiImg, bool SHOWGUI)
{
	...
	// Perform candidate ball detector using box.
	...
	// Perform circle detector from candidate boxs.
	...
	for (size_t i = 0; i < circles.size(); i++)
	{
		...
		if (!checkDistance_Rec(recCircle, projection, realPos))
		{
			...
			//TODO: Distance size check is skipped. Check this.
			//continue;
		}
		else
		{
	...
	if (params.ball->enableHog() && resBall.size() < 1)
	{
		for (size_t iCan = 0; iCan < CandidateBall.size(); iCan++)
		{
			...
			int minSizeDim = max(10,
					(int) (max(rec.size().height, rec.size().width) / 17.));
		...
		CameraProjections &projection, Point2f &realPos)
{
	...
	{
		ROS_ERROR("checkDistance_Rec: Error in programming");
		return false;
	}
	...
	if (!rangeFound)
	{
		...
		{
			ROS_ERROR("checkDistance_Rec: Programming Error!");
		}
	}

		...
		Rect box, vector<LineSegment> &resLines)
{
	...
	//
	// COUNT_MID_P equally spaced points are chosen on each line segment under review,
	// and two normals to the line are constructed at each of these points. Those points are saved in queryPoint.
	//
	for (size_t i = 0; i < linesFromHoughP.size(); i++)
	{
		...
		for (size_t j = 0; j < COUNT_MID_P; j++)
		{
			...
			// These are the two normals from the mid point.
	...
	//
	// Iterate through all lines and compute vote value.
	//
	...
	for (size_t i = 0; i < allLines.size(); i++)
	{
		...
		for (size_t j = 0; j < COUNT_MID_P; j++)
		{
					...
					&& itDown.count >= jumpMin)
			{
				...
				// Iterate from mid to up point of the normal line.

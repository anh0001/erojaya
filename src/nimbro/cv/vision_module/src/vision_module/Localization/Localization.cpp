		...
		vector<FeatureContainer> &AllFeatures)
{
	...
	double UPDATESTRONG = params.loc->UPDATESTRONG()
			* params.loc->TOTALGAIN();
	double UPDATEWEAK = params.loc->UPDATEWEAK()
			* params.loc->TOTALGAIN();
		...
		if (lineSeg.GetLength() > params.loc->minLineLen())
		{
			...
			else
			{
				...
				if (goalPosition.size() >= 2
						&& lineSeg.DistanceFromLine(goalPosition[0]) < 0.75
						&& lineSeg.DistanceFromLine(goalPosition[1]) < 0.75)
				{
	...
	// This is to determine whether line is front or back line of goal when the line type is HorUndef.
	...
	{
		updateVertexIdx();
		if ((nodeCounter % params.loc->lineBufferCounterToUpdate() == 0) && PreviousVertexId > 0)
		{

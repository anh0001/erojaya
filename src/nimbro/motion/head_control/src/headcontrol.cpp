...
// Constants
const std::string HeadControl::RESOURCE_PATH = "/robotcontrol/headcontrol/";
...
HeadControl::HeadControl()
 ...
 , m_yawOffset(CONFIG_PARAM_PATH + "yaw/offset", -M_PI, 0.02, M_PI, 0.0)
 ...
 , m_pitchOffset(CONFIG_PARAM_PATH + "pitch/offset", -M_PI, 0.02, M_PI, 0.0)
 , m_yawTarget(0.0)
 , m_yawCurX(0.0)
 , m_yawCurV(0.0)
 , m_yawCurEffort(0.0)
...
void HeadControl::step()
{
	...
	//m_yawSpline.setParams(m_yawCurX, m_yawCurV, m_yawTarget+m_yawOffset(), 0.0, m_yawMaxVel(), m_yawMaxAcc());
	//m_pitchSpline.setParams(m_pitchCurX, m_pitchCurV, m_pitchTarget+m_pitchOffset(), 0.0, m_pitchMaxVel(), m_pitchMaxAcc());
	...
	else
	{
		...
		{
			yawCmdX = m_yawTarget;
			//yawCmdX = m_yawTarget+m_yawOffset();
			yawCmdV = 0.0;
		}
		...
		if(pitchTime < dT) // Pitch is within reach of its target
		{
			...
			//pitchCmdX = m_pitchTarget+m_pitchOffset();
			pitchCmdV = 0.0;
		}
...
void HeadControl::handleLookAtTarget(const head_control::LookAtTargetConstPtr& msg)
{
	...
	{
		//TODO: Check this!!! Added offset to angular data
		m_yawTarget   = msg->vec.z + m_yawOffset();
		m_pitchTarget = msg->vec.y + m_pitchOffset();
		if(msg->is_relative)
		{

...
bool FilteredListener::waitForRelativeToNowTransform(const std::string& target_frame, const std::string& source_frame, double shift, double timeout)
{
	...
	while((ros::WallTime::now() - startWallTime).toSec() <= timeout)
	{
		...
		try
		{
			bool res = canTransform(target_frame, source_frame,
									ros::Time::now() - ros::Duration(shift),
									&error_msg);
			if(res)
			{
				return true;
			}
			ROS_WARN("Transform isn't available: %s", error_msg.c_str());
		}
		catch(...)
		{
			ROS_ERROR("Transform isn't available: %s", error_msg.c_str());
		}
		usleep(10000);
	}

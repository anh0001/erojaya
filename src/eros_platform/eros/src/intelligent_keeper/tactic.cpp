...
//			DebugAI(debug_print);
			...
			if(vPos<1.5) 					{arahTendang=6;initialPosition=1;}
			else if(vPos>=1.5 && vPos<1.8)	{arahTendang=5;initialPosition=1;}
			...
			else if(vPos>=4.4)				{arahTendang=16;initialPosition=5;}
...
void PositionGenerator(){
	...
	{
		relativePosition0+=getPosition;
		counterpos1++;
		if (counterpos1>100)
		{
			stepG=0;
			counterpos1=0;
			relativePosition0=relativePosition0/100;
			fprintf(stderr, "POS--->>[%1.2f]\n",relativePosition0);
			step++;
			//fprintf(stderr, "1\n");
		}
	}else if(stepG==2){
	...
	}else if (stepG==5)
	{
		...
		fprintf(stderr, "POS--->>[%1.2f][%1.2f]>>>[%1.2f]\n",relativePosition1,relativePosition2,vPos);
	}

	motion=0;
	motionAct(xBall, yBall,0, 10);
	//fprintf(stderr, "sG[%d]cp[%d]\n",stepG,counterpos1);
}
*/
void lostball_keeping(){
	...
	if(nBall == 1)
	{
		...
		if(countn>300)
		{		
			if(((1 <= arahLihat <= 4) || (17 <= arahLihat <= 20)) && sdtbolay <= 70) 
			{
				//motion=10;
				CountAda++;
				if(CountAda>300) step++;
			}
			else motion=0;
		}	
	}
...
void TaktikBacktoPos_keeping(){
	...
	switch(stepK){
		...
		case 1: if(dtaXPOS>19) motion=14;
			else if(dtaXPOS<-19) motion=13;
			else stepK++;	break;
		case 2: if(dtaYPOS>0) motion=20;
			else stepK++;	break;
		case 3: if(dtaXPOS>19) motion=14;
			if(dtaXPOS<-19) motion=13;
			else 
			{
...
void TaktikBacktoPos_keeping(){
	ethreadsearching = 0;
	switch(stepK){
		case 0: if(arahRobot>11) motion=12;
			else if(arahRobot<12 && arahRobot>1)	motion=11;	
			else stepK++;	break;
		case 1: if(dtaXPOS>19) motion=13;
			else if(dtaXPOS<-19) motion=14;
			else stepK++;	break;
		case 2: if(dtaYPOS>0) motion=19;
			else stepK++;	break;
		case 3: if(dtaXPOS>19) motion=13;
			if(dtaXPOS<-19) motion=14;
			else 
			{
				if(arahRobot>11) motion=12;
				else if(arahRobot<12 && arahRobot>1)	motion=11;	
				else step=stepK=0;	break;
			}
	}
	motionAct(sdtblax,sdtexcute,motion,10);
}
	...
	if(nBall == 1)
	{
		...
		{		
			...
			else if(arahLihat >= 6 && arahLihat <= 15)
			{
				...
				{
					countn=0;	
					fprintf(stderr,"bola samping belakang");
					step++;
					flagsudahdekat=0;
				}
			}
	...
	{	
		...
		switch(langkah)
		{
			case 0:	if(arahRobot==1)	
				...
				else if(arahRobot>11)
				{			
					motion=12;
				}
				else if(arahRobot<12 && arahRobot>1)
				{			
					motion=11;
				}
				break;
			case 1:	if(arahRobot==11)	
				{
					if(countmundur<300)
					{
						motion=19;
...
void TaktikkeBola3(int arahhadap)
{
	...
	if(nBall == 1)
	{
		...
		if(sdtbolay <= 30 && sdtbolax<15 && sdtbolax>-15 && (arahRobot <= 2 || arahRobot>=20)) //arahRobot==arahhadap
		{
			countertimeout++;
			fprintf(stderr,"====wes 2\n");
...
void TaktikNggiring3()
{
	...
	if (nBall == 1 ){
		...
		if(arahRobot == arahNggiring && arahRobot==arahLihat){
			...
			if(dtaYPOS>1200 || flagStopDribble==true){
				flagStopDribble=true;
				fprintf(stderr, "wesss\n");
				resetOdometry=true;
				motion=0;
			}
		}
...
void TaktikBertahan()
{
	...
	aktifkansearching();
	countForward++;
	if(countForward<1000){
		motion = 20;
		
		if(arahRobot>1 && arahRobot<11){
			motion = 12;
		}
		else if(arahRobot<20 && arahRobot>10){
			motion = 11;
		}
	}
	else if (arahRobot=1){
...
// void TaktikLuruskanGW_wide(int sudutputar)
// {
// 	aktifkansearching();
// 	int madepgawang=CariArahGawang(sudutputar,0);
// 	if(sudutputar==0)madepgawang=1;
// 	else if(madepgawang>4 && madepgawang<11)madepgawang=4;
// 	else if(madepgawang<17 && madepgawang>11)madepgawang=17;
...
// 					}
// 					if(arahRobot==madepgawang && sdtbolax>-5 && sdtbolax<5){
// 						arahNggiring=madepgawang;
// 						//TrackGawang dan Bola Langsung
// 						if(sudahlihatGW==2){step=6;sudahlihatGW=0;}
// 						else if(counterobseks>=100){countDribb=countDribbChange=2000;counterobseks=0; step=5;}	
// 						else if(madepgawang<=18 && madepgawang>=12){countDribb=3000;step=5;}
// 						else if(madepgawang>=4 && madepgawang<=11){countDribb=2000;step=5;}
// 						else if(madepgawang==19 || madepgawang==3){countDribb=1500;step=5;}
// 						else if(madepgawang==20 || madepgawang==2){
// 							if(posrobotY<=2)step=6;
// 							else {step=5;countDribb=3500;}//sedang gawang, miring
// 						}
// 						else { //arah 1
// 							if(posrobotY<=2)step=6;
// 							else {step=5;countDribb=3500;}//jauh gawang
// 						}
// 						motion = 10;
// 						suduteksekusi = countLihatblg = 0;
// 						CountDelay=0;
...
// 		}
// 		CountLost=0;
// 		if(sdtbolay>45 || sdtbolax > 40 || sdtbolax < -40) {
// 			countLihat++;
// 			//yBall=sdtexcute;
// 			//xBall=sdtblax;

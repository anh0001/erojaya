...
unsigned int i;
String ball_cascade_name = "/home/eros/new_plt/classifier/cascade.xml"; //lokasi file ball_cascade
CascadeClassifier ball_cascade; //class for load classifier from file (ball_cascade)
...
void VisionPublish(){
	...
	dta_vision_it.data.push_back(ball[i].x);
	dta_vision_it.data.push_back(ball[i].y);
	...
	dta_vision_it.data.push_back(Mrow);
	dta_vision_it.data.push_back(Mrow);
    ROS_INFO("I send: [%d][%d][%d][%d]",ball[i].x,ball[i].y,distBall,dataYH1);
}
...
void detectAndDisplay( Mat frames )
{
    ...
    Mat frame_gray;
    vector<int> num_detection(1,0);
    cvtColor( frames, frame_gray, COLOR_BGR2GRAY );
    equalizeHist( frame_gray, frame_gray );
    if(distBall==1){
    	ball_cascade.detectMultiScale( frame_gray, ball, 1.1, 3, 0|CASCADE_SCALE_IMAGE, Size(70, 70));
    }else if(distBall==2){
    	ball_cascade.detectMultiScale( frame_gray, ball, 1.1, 3, 0|CASCADE_SCALE_IMAGE, Size(40, 40));
    }else{
    	ball_cascade.detectMultiScale( frame_gray, ball, 1.1, 3, 0|CASCADE_SCALE_IMAGE, Size(16, 16));
    }

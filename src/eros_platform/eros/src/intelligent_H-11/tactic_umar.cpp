		...
		if(stepG==0){
			...
			if (counterpos1>max_counterpos1 && flagcounterpos2==false){
				...
				if(relativePosition0>=3){
					batasGiringY=2200;//changeable value
					arahTendang=1;
					stepBuffer=5; //Nggiring dulu--> Nendang
					flagcounterpos2=1;
					initialPosition=10;
					...
					batasGiringY=1500;//changeable value
					arahTendang=1;
					stepBuffer=5; //Nggiring dulu--> Nendang
					flagcounterpos2=1;
					initialPosition=10;
			...
			int max_counterpos1=180;//changeable value
			...
			int max_counterpos1=230;//changeable value
			yBall=sdtdefy;
			xBall=generatesdtX(-90);
			posDir=3;
			flagLocalize=1;
			...
			if(relativePosition1>5)relativePosition1=relativePosition2;
			...
			if(vPos<1.5) 					{arahTendang=5;initialPosition=1;}//step=8;kakiTendang=2;}
			...
			else if(vPos>=4.4)				{arahTendang=17;initialPosition=5;}//step=8;kakiTendang=1;}
...
void TaktikkeBola4(int arahhadap){
	...
	if(nBall == 1){
		...
		if(sdtbolay <= 35 && sdtbolax<15 && sdtbolax>-15 && (arahRobot == rotasiarah(arahhadap)   || 
															 arahRobot == rotasiarah(arahhadap-1) ||
															 arahRobot == rotasiarah(arahhadap+1) ))
		{
			countertimeout++;
				...
				if(flagNyaduk1 == true) { 				
					if(initialPosition>5){
						...
						if(dtaYPOS<-800){
							...
							initialPosition=7;
							batasGiringY=0; //langsung reset odometry, karena giring sudah terpenuhi
...
void lostball_positioning(){
	int MIN_MAIN_KIRI=-3500,MAX_MAIN_KIRI=500;
	int MIN_MAIN_KANAN=-1200,MAX_MAIN_KANAN=2400;
	aktifkansearching();
	if(nBall == 1){
		countn++;
		...
		if(mainx==KIRI){
			if(arahRobot<=20 && arahRobot>=11){ // Lepaskan robot dari kanan
				//16
				initialPosition=10;
				if(arahRobot>16-serongLost)motion=11;
				else
				if(arahRobot<16-serongLost)motion=12;
				else {
					if(majulost<1000){
						motion=20;
					}else if(majulost>=1000){
			...
			if(arahRobot>=1 && arahRobot<11){ //Lepaskan robot dari kiri
				//6
				initialPosition=9;
				if(arahRobot>6-serongLost)motion=11;
				else
				if(arahRobot<6-serongLost)motion=12;
				else {
					if(majulost<1000){
						motion=20;
					}else if(majulost>=1000){

...
void strategi_serang_WIDE4(){
	...
	if ((laststep==6 && step==7) || (laststep==9 && step==7)){
		arahInitial=0;
		bufferTendang=rotasiarah((-1*((int)(dtaXPOS/500)))+arahTendang);
	...
	switch(step)
	{
		case 0:  Initialize();break;
		case 1:  lostball();break;
		case 2:  //if(initialPosition>5)TaktikkeBola4(1);
				 //else{
				bufferTendang=rotasiarah((-1*((int)(dtaXPOS/500)))+arahTendang+arahInitial);
...
void strategi_bertahan_cpp(){
	...
	{
		case 0: Initialize();break;
		case 1: bufferTendang=rotasiarah((-1*((int)(dtaXPOS/400)))+arahTendang);
				if(bufferTendang>5 && bufferTendang<11)bufferTendang=4;
				else if(bufferTendang<18 && bufferTendang>=11)bufferTendang=18;
...
void strategi_penalty_WIDE(){
	...
	else if(dtaFall==3){step=5;countLihat=0;} //Saat robot menendang
	...
	switch(step)
	{
		...
		case 2: bufferTendang=rotasiarah((-1*((int)(dtaXPOS/500)))+arahTendang+arahInitial);
				if(bufferTendang>5 && bufferTendang<11)bufferTendang=4;
				else if(bufferTendang<18 && bufferTendang>=11)bufferTendang=18;
				TaktikkeBola_pinalty(bufferTendang);
				break;
		case 3:  
				bufferTendang=rotasiarah((-1*((int)(dtaXPOS/500)))+arahTendang+arahInitial);
				if(bufferTendang>5 && bufferTendang<11)bufferTendang=4;
				else if(bufferTendang<18 && bufferTendang>=11)bufferTendang=18;
				TaktikLuruskanGW_pinalty();
				break;
		case 4: TaktikEksekusi4_pinalty();break;
		case 5: TaktikPEksekusi();break;		
		case -1: lostball_positioning();break;
		case -2: 	aktifkansearching();
					motionAct(xBall,yBall,0,0);
					if(dtaFall==2){
...
void bermain(){
	...
	switch (refree){
		...
		case 5: //State Ready for Kickoff
		{
			...
			if(dtaXPOS!=0 && resetOdometry==false)bufferTendang=rotasiarah((-1*((int)(dtaXPOS/400)))+arahTendang);
			resetOdometry=true;
			aktifkansearching();
			play=sek=0;
			motion = 0;
...
void init_positioning(){
	int MIN_MAIN_KIRI=-3300,MAX_MAIN_KIRI=500;
	int MIN_MAIN_KANAN=-1200,MAX_MAIN_KANAN=3300;

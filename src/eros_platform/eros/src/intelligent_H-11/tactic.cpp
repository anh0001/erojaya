...
void TaktikkeBola_pinalty(int arahhadap){
	...
	if(nBall == 1){
		...
		{
			countertimeout++;
			sprintf(debug_print,"[TaktikkeBola_penalty] :: I'm already to kick the ball [%d]",countertimeout);
			DebugAI(debug_print);
			if(countertimeout>50){
				countertimeout=0;
				sprintf(debug_print,"[TaktikkeBola_penalty] :: Tendang dulu bro 1 [%d]",counterNyaduk);
				DebugAI(debug_print);				
				if(true){
					if(false){
						batasGiringY=0;

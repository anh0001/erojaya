...
float KNN_Basic(int x0, int x1, int y0, int y1, int direction){
	...
	int j=0;
	int KNN_DTemp[3];//alamat,distance,posisi
	for(int k=0; k<MAX_DATA; k++){
		KNN_DTemp[1]=0,KNN_DTemp[0]=j;
		for(int i=j; i<MAX_DATA; i++){
			if(KNN_Distance[i][1]>KNN_DTemp[1]){
				KNN_DTemp[1]=KNN_Distance[i][1];
				KNN_DTemp[0]=i;
				KNN_DTemp[2]=KNN_Distance[i][0];
			}
		}
		KNN_Distance[KNN_DTemp[0]][0]=KNN_Distance[j][0];
		KNN_Distance[KNN_DTemp[0]][1]=KNN_Distance[j][1];
		KNN_Distance[j][0]=KNN_DTemp[2];
		KNN_Distance[j][1]=KNN_DTemp[1];
		j++;	
	}
    ...
    if(flagLocalize){
		...
		if(maxLine>3){
			...
			for(int i=0; i<maxLine; i++){
				...
				//90 derajat
				if(posDir==1){
					if(grad[i]<-0.5 || grad[i]>0.5)continue;
					if(lastgrad==0)IPoint=i;
					if(fabs(lastgrad)>grad[i])IPoint=i;
					IPoint=i;
				}else if(posDir==2){
			...
			//fprintf(stderr,"{%d,%d,%d,%d,3},",dtaFromField[0+(IPoint*4)],dtaFromField[1+(IPoint*4)],dtaFromField[2+(IPoint*4)],dtaFromField[3+(IPoint*4)]);
			//fprintf(stderr, "\n");
...
void DebugPublish(int param1, int param2, int param3, int param4){

...
void TaktikBertahan(){
	...
	if(nBall == 1){
		...
		{
			countertimeout++;
			sprintf(debug_print,"[TaktikkeBola] :: I'm already to defend the ball [%d]",countertimeout);
			DebugAI(debug_print);
...
void TaktikBertahan_off(){
	...
	if(nBall == 1){
		...
		motion = GerakHadapBola(sdtbolax,sdtbolay-40,arahRobot,arahhadap);
		if(sdtbolax>15)	motion=26;
		else if(sdtbolax<-15)	motion=25;
		if(motion==17 || motion==18) motion=10;
...
void TaktikkeBola3(int arahhadap){
	...
	if(nBall == 1){
		...
		if (sdtbolay >= 40 && arahRobot <= 13 && arahRobot >= 7){
			counterTerobos++ ;
			motion = GerakTerobosBola(sdtbolax,sdtbolay,arahRobot,arahhadap);
			sprintf(debug_print,"[TaktikkeBola] :: Maju Terobos step 1");
			DebugAI(debug_print);
			if (counterTerobos > 100) flagCuri = true ;
				if (flagCuri= true && (arahLihat <= 12 && arahLihat >= 8) && (arahRobot <= 16 && arahRobot >= 14) || (arahRobot <= 6 && arahRobot >= 4))
				{
					countRebut++ ;

	...
	if(play == 1 || play == 2){
		...
		if(state2nd!=0 && dtComm!=9){
			...
			switch(dtComm){
				...
				case 9 : step=-1; catchflagsama=0; dtJob=0;resetOdometry=true; motionAct(sdtblax,sdtdefy,0,0); break;
...
void init_positioning(){
	int MIN_MAIN_KIRI=-1800,MAX_MAIN_KIRI=500;
	int MIN_MAIN_KANAN=-1200,MAX_MAIN_KANAN=1800;
	
	if(arahRobot<=20 && arahRobot>=11){ // Lepaskan robot dari kanan
		//16
		initialPosition=10;
		if(arahRobot>16)motion=11;
		else

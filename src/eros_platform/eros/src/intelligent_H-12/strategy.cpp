...
void strategi_serang_WIDE4(){
	...
	switch(step)
	{
		...
		case 2:  //if(initialPosition>5)TaktikkeBola4(1);
				 //else{
				bufferTendang=rotasiarah((-1*((int)(dtaXPOS/400)))+arahTendang);
		...
		case 7:  TaktikPEksekusi();break;
		case 8:  TaktikEksekusiSamping();break;
		case 9:  TaktikLuruskanGW_wide_rokh();break;
		case -1: lostball_positioning();break;
		case -2: 	aktifkansearching();
					motionAct(xBall,yBall,0,0);
					if(dtaFall==2){
...
void strategi_penalty_WIDE(){
	...
	switch(step)
	{
		...
		case 2: 
				...
				// if(bufferTendang>5 && bufferTendang<11)bufferTendang=4;
				// else if(bufferTendang<18 && bufferTendang>=11)bufferTendang=18;
				TaktikkeBola_pinalty(arahNggiring);
				break;
		case 3:  
				// bufferTendang=rotasiarah((-1*((int)(dtaXPOS/400)))+arahTendang);
				// if(bufferTendang>5 && bufferTendang<11)bufferTendang=4;
...
void bermain(){
	...
	fprintf(stderr,"aR[%3d]ref[%2d]nBall[%d]aN[%d]bT[%d]dF[%d]\n",arahRobot,refree, nBall, arahNggiring, bufferTendang,dtaFall);
